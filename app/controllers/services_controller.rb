class ServicesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @service.save
        ...
        ActionCable.server.broadcast('service_channel', "#{@service.title}")
        
      else
        format.html { render :new, status: :unprocessable_entity }
        format.json { render json: @service.errors, status: :unprocessable_entity }
        format.turbo_stream { render turbo_stream: turbo_stream.replace(@service, partial: 'services/form', locals: { service: @service }) }
      end
    end
  end
  ...
  private
    ...
    def service_params
      params.require(:service).permit(:title, :completed)
    end
end
